...
const RESULT_SCREEN_HIGH_SCORED_ID = "js-high-scores";
...
class ResultScreen extends Common {
  ...
  bindToElements() {
    ...
    this.uesrPointsElement = this.bindToElement(RESULT_SCREEN_USER_POINTS_ID);
    this.highScoresElement = this.bindToElement(RESULT_SCREEN_HIGH_SCORED_ID);
  ...
  viewResultScreen(isGameWin, playerPoints, level) {
    ...
    this.resultTextElement.textContent = isGameWin ? "wygrałes" : "przegrałes";
    this.uesrPointsElement.textContent = String(playerPoints);
    this.highScoresElement.textContent = String(userData.getHightScores(level));
  }
